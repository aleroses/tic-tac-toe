# 🎮 Tic Tac Toe — Mario Edition

A fun and colorful **Tic Tac Toe** game built with **Vite + React (JavaScript)**.  
Inspired by the world of **Mario Bros**, it brings a nostalgic look with modern web technologies.  

This project is **open source** — everyone is welcome to contribute!

## 🚀 Demo

👉 [Play the game online](https://aleroses.github.io/tic-tac-toe/)

## 🧩 Features

- Built with **React + Vite**
- Clean and modular component architecture
- Simple game logic using React Hooks
- Color palette inspired by **Mario Bros**
- Responsive design (mobile, tablet, desktop)
- Light, fast, and fully open source 🎯

## 🛠️ Installation & Setup

Follow these steps to run the project locally:

```bash
# 1. Clone the repository
git clone https://github.com/aleroses/tic-tac-toe.git

# 2. Move into the project folder
cd tic-tac-toe-react

# 3. Install dependencies
npm install

# 4. Start the development server
npm run dev
```

Then open your browser and go to:
👉 http://localhost:5173/

## 🤝 Contributing

Contributions are welcome! 💡  
If you’d like to improve this project, follow these steps:

1. Fork the repository
2. Create a new branch (git checkout -b feature-name)
3. Commit your changes (git commit -m 'Add a new feature')
4. Push the branch (git push origin feature-name)
5. Open a Pull Request

Please keep your code clean and follow consistent naming conventions.

